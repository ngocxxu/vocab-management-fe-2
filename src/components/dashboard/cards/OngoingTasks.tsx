    ...
    <Card className="overflow-hidden border-0 bg-white shadow-lg">
      <CardHeader className="border-b border-slate-100 pb-4">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-xl font-bold text-slate-900">On Going Task</CardTitle>
            <p className="text-sm text-slate-600">Best performing employee ranking.</p>
          </div>
          <div className="flex items-center space-x-3">
            <div className="relative">
              <Search className="absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-slate-400" />
              <Input placeholder="Search..." className="h-10 w-48 rounded-xl border-slate-200 pl-10 focus:border-blue-500" />
            </div>
      ...
      <CardContent className="p-6">
        <div className="space-y-4">
          {tasks.map(task => (
            <div key={task.id} className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 transition-all duration-200 hover:bg-slate-50 hover:shadow-md">
              <div className="flex items-center justify-between">
