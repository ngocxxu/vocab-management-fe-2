...
export function MessageBubble({ message, onRetry }: MessageBubbleProps) {
  ...
  if (isUser) {
    ...
    return (
      <div className="flex min-w-0 flex-col items-end gap-1 px-4">
        <div
          className={cn(
            'min-w-0 max-w-[75%] break-words whitespace-pre-wrap rounded-2xl rounded-tr-sm bg-primary px-4 py-2.5 text-sm text-primary-foreground',
  ...
  return (
    <div className="flex min-w-0 items-start gap-2 px-4">
      ...
      />
      <div className="flex max-w-[75%] min-w-0 flex-col items-start gap-1">
        <div className={cn(
          'min-w-0 max-w-full overflow-x-hidden rounded-2xl rounded-tl-sm bg-muted px-4 py-2.5 text-sm break-words text-foreground',
          '[&_p]:mb-1 [&_p:last-child]:mb-0 [&_ul]:my-1 [&_ol]:my-1 [&_li]:my-0',
          '[&_strong]:font-semibold [&_code]:rounded [&_code]:bg-muted [&_code]:px-1 [&_code]:py-0.5 [&_code]:text-xs [&_code]:break-all [&_code]:text-foreground',
