            ...
            <div className={`grid gap-4 ${vocab.textTargets.length >= 2 ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'}`}>
              {vocab.textTargets.map((target: TVocab['textTargets'][number], index: number) => {
                const subjects = target.textTargetSubjects
                  ?.map(tts => tts.subject)
                  .filter((subject): subject is TSubjectBadge => Boolean(subject?.id && subject.name)) ?? [];
                  ...
                  <div key={`${target.textTarget}-${index}`} className="rounded-lg border border-border bg-muted/20 p-4">
                    <div className="mb-3 flex items-start justify-between gap-2">
                      <div className="flex min-w-0 flex-1 items-center gap-2">
                        <h4
                          className="max-w-48 min-w-0 shrink-0 truncate text-lg font-bold text-foreground sm:max-w-64 md:max-w-80 lg:max-w-96"
                          title={target.textTarget}
                        >
                          {target.textTarget}
                        </h4>
                        <Button
